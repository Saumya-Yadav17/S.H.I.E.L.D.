...
*   **The "Swivel Chair" Effect:** To investigate a single potential threat, an analyst must switch between 10+ distinct tools (Email Gateway, SIEM, Firewall logs, Threat Intelligence feeds, Detonation Sandboxes, GRC Dashboards, Forensics Labs).
...
*   **Malware Agent:** Simulates detonating attachments in a sandbox to check file hashes against Threat Intel.
*   **Insider Threat Agent:** Simultaneously scans internal user logs for data exfiltration patterns (e.g., large uploads to non-compliant IPs).
...
## 4. Enterprise Capabilities (Beyond the Agent)
I expanded the project to demonstrate how an AI Agent integrates into a full Enterprise environment:

*   **Unified Operations:** Merged Network Ops (ACLs/VPNs), IDS Management, and Forensics into one UI.
*   **Governance, Risk & Compliance (GRC):** Implemented **FAIR Model** Risk Analysis (Monte Carlo simulations) and **STRIDE/LINDDUN** Threat Modeling visualizations.
*   **Open Source Integrations:** The platform features a curated toolkit of open-source security tools (Heisenberg, VulnRisk, Strix) that can be deployed directly from the dashboard.
*   **Live Data Ingestion:** Built a custom `GmailTool` that allows the agent to switch from "Simulation Mode" to "Live Mode," scanning a real user's inbox via the Gmail API.
...
I migrated the logic to a **React-based "Cyberpunk" Interface** to visualize the AI's "brain" (the *ScannerVisual* component). The biggest challenge was moving from mock data to real data. I overcame this by building a **Dual-Mode Ingestion Engine** that can seamlessly switch between simulated enterprise traffic (AWS CloudTrail, CrowdStrike logs) and live real-world data.
